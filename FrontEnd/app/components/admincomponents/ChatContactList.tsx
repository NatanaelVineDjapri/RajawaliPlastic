...
const mapConversationToContact = (
  conv: ConversationData,
  currentAdminId: string
): ChatContact | null => {
  const potentialCustomer =
    conv.sender?.id === currentAdminId ? conv.receiver : conv.sender;
  ...
  return {
    ...
    name: customer.name,
    lastMessageText:
      conv.message.substring(0, 40) + (conv.message.length > 40 ? "..." : ""),
...
const ChatContactList: React.FC<ChatContactListProps> = ({
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && window.Echo) {
      listenToChannel(
        ADMIN_ID,
        (event: { message: MessageData; sender: UserData }) => {
  ...
  return (
    <div className="w-100 overflow-y-auto">
      <h5 className="p-3 mb-0 border-bottom bg-light">
        Daftar Chat ({contacts.length})
      </h5>
      <div className="chat-contact-list-inner">{renderContacts()}</div>
    </div>
  );
};
