  ...
  {isLoadingMessages ? (
    ...
    messages
      ...
      .map((msg, index, arr) => {
        ...
        const prevMsgDate = index > 0 ? new Date(arr[index - 1].timestamp || 0) : null;
          ...
          <React.Fragment key={msg.id}>
            {isNewDay && (
              <div className="text-center text-muted my-2">
                {msgDate.toLocaleDateString()}  {/* Format tanggal */}
              </div>
            )}
            <div className={`d-flex mb-3 ${msg.from === 'admin' ? 'justify-content-end' : 'justify-content-start'}`}>
              <div className={`p-2 rounded shadow-sm ${msg.from === 'admin' ? 'bg-primary text-white' : 'bg-white text-dark'}`}
                   style={{ maxWidth: '75%', wordBreak: 'break-word', whiteSpace: 'pre-wrap', opacity: msg.isTemp ? 0.6 : 1 }}>
                {msg.text}
