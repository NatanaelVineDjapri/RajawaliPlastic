...
const ChatBox: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const handleMessageReceive = (event: { message: MessageData }) => {
            ...
            const isImageMessage = !newMsg.message && messageId && newMsg.sender_id === adminId;
            ...
            setMessages(prev => prev.some(m => m.id === messageId) ? prev : [...prev, placeholderMsg]);
    ...
    return (
        <div className="chat-box">
            {authError && <div className="p-2 text-danger text-center border-bottom">{authError}</div>}
                ...
                <div className="chat-messages">
                    {messages.slice().sort((a, b) => a.id.localeCompare(b.id)).map(msg => (
                        <div key={msg.id} className={`message-row ${msg.from === 'user' ? 'from-user' : 'from-bot'}`}>
                            <div className={`message-bubble ${msg.from} ${msg.isTemp ? 'opacity-50' : ''}`}>
                                {msg.imageUrl && (
                                    <img
                                        src={msg.imageUrl}
                                ...
                                {msg.isTemp && msg.text.includes('[Mengirim gambar...]') && (
                                    <span style={{ fontStyle: 'italic', opacity: 0.8 }}>{msg.text}</span>
                                )}
                            </div>
                        </div>
                    ))}
